...
<div class="max-w-xl">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Edit ODC</h2>
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        @if($errors->any())
            <div class="bg-red-50 text-red-600 text-sm rounded-lg px-4 py-3 mb-4">{{ $errors->first() }}</div>
        @endif
        <form method="POST" action="{{ route('odc.update', $odc) }}">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-sm text-gray-600 mb-1">Kode ODC</label>
                <input type="text" name="kode_odc" value="{{ old('kode_odc', $odc->kode_odc) }}"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_odc" value="{{ old('nama_odc', $odc->nama_odc) }}"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="jumlah_port" value="{{ old('jumlah_port', $odc->jumlah_port) }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm text-gray-600 mb-1">Latitude</label>
                    <input type="text" name="latitude" value="{{ old('latitude', $odc->latitude) }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm text-gray-600 mb-1">Longitude</label>
                    <input type="text" name="longitude" value="{{ old('longitude', $odc->longitude) }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            <div class="mb-6">
                ...
                <textarea name="keterangan" rows="2"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('keterangan', $odc->keterangan) }}</textarea>
            </div>
