...
<div class="max-w-xl">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Tambah ODP</h2>
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        @if($errors->any())
            <div class="bg-red-50 text-red-600 text-sm rounded-lg px-4 py-3 mb-4">{{ $errors->first() }}</div>
        @endif
        <form method="POST" action="{{ route('odp.store') }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm text-gray-600 mb-1">Kode ODP</label>
                <input type="text" name="kode_odp" value="{{ old('kode_odp') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="ODP-A1-01" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm text-gray-600 mb-1">Nama ODP</label>
                <input type="text" name="nama_odp" value="{{ old('nama_odp') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="ODP Jl. Mawar No.1" required>
            </div>
            <div class="mb-4">
                ...
                <select name="odc_id"
                    ...
                    @foreach($odc as $o)
                        <option value="{{ $o->id }}" {{ old('odc_id') == $o->id ? 'selected' : '' }}>
